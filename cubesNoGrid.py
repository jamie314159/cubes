...
possibles = []
class Square:
	def __init__(self, xIn, yIn, master = 0):
		self.master = 0
		self.x = xIn
		self.y = yIn
		self.draw()
	...
	def draw(self, master=b):
		...
		x2 = (self.x+2)*SCALE
		y2 = (self.y+2)*SCALE
		self.drawing = b.create_rectangle(x1, y1, x2, y2, width=2)

	def move(self, x, y):
		self.x = x
		self.y = y
		b.erase(self.drawing)
		self.draw()
...
squares.add(Square(x,y, master=1))
# g.add(x,y)
